...
## Mode Selection (Wide vs Refinement)

If user wants *incremental refinement* (likes current site, wants section-by-section upgrades, baseline vs variants, token/motion tuning) → run `design-section-labs` instead.

If user wants a *new direction* (redesign, fresh aesthetic, multiple divergent themes) → continue with this skill.
...
- Respect existing brand fonts/colors first; only change deliberately.
