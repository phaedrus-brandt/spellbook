...
Global memory in `~/.codex/design-memory.db` (SQLite, auto-created).

```bash
ENGINE="$HOME/.codex/skills/evolve/engine.py"
...
```
Load ~/.codex/skills/aesthetic-system/references/taste-frontend.md and follow strictly.
...
Global design memory persists across sessions at `~/.codex/design-memory.db`.
...
| `~/.codex/design-memory.db` | `.design-evolution/export.json` |
